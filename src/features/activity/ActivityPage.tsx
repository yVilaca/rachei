    ...
    for (const debt of debts) {
      ...
      if (isCreditor) {
        list.push({
          ...
          sub: `${gName} · ${formatCurrency(debt.totalAmountCents)} · ${relativeDate(debt.createdAt)}`,
          date: debt.createdAt,
          actionable: false,
          debtId: debt.id,
        })
      ...
      for (const inst of debt.installments) {
        ...
        if (inst.status === 'paid' && inst.confirmedAt) {
          if (isCreditor && inst.debtorUserId !== currentUser.id) {
            list.push({
              ...
              sub: `${debt.description} · ${gName} · ${formatCurrency(inst.amountCents)} · ${relativeDate(inst.confirmedAt)}`,
        ...
        if (inst.status === 'pending' && isMyInst && !isCreditor) {
          list.push({
            ...
            title: `Você deve ${formatCurrency(inst.amountCents)} a ${creditorUser?.name ?? 'alguém'}`,
        ...
        if (inst.chargeLink && isMyInst && inst.status === 'pending') {
          list.push({
            ...
            sub: `${debt.description} · ${formatCurrency(inst.amountCents)} · ${relativeDate(inst.chargeLink.expiresAt)}`,
            date: debt.createdAt,
            actionable: true,
            debtId: debt.id,
            installmentId: inst.id,
